...
void tx_task(){
	...
	while(1)
	    {
			// 네트워크 준비 대기 (rx_task와 같은 방식)
			while (udp_sock < 0) {
				printf("tx_task: waiting for udp_sock\n");
				vTaskDelay(pdMS_TO_TICKS(100));
			}

			while(1)
			{
				// 제어 명령이 새로 들어올 때까지 대기
				if (xQueueReceive(xControlOutQueue, &tx_to_unreal, pdMS_TO_TICKS(100)) == pdTRUE)
				{
